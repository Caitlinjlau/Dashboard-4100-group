...
author: '[[Caitlin Lau, CJL236; Destiny Malloy DAM359]]'
...
all_data = inner_join(quiz,experience, by="STUDENT_KEY")
all_data
...
#score per quiz
student_quiz_scores <- all_data %>%
  group_by(STUDENT_KEY,QUIZ_NUMBER) %>%
  summarise(
    score = QUIZ_SCORE
  ) %>%
  arrange(STUDENT_KEY, QUIZ_NUMBER)

student_quiz_scores <- distinct(student_quiz_scores)

student_quiz_scores <- subset(student_quiz_scores, !is.na(score))
...
test = subset(all_data , STUDENT_KEY == 2) %>%
  summarise(
...
lectures_per_year <- all_data %>%
group_by(YEAR.x) %>%
  summarise(
    total_lectures = max(SESSION_NUMBER)
  )

lectures_attended <- all_data %>%
  group_by(STUDENT_KEY) %>%
  summarise(
    n = sum(ATTENDED)
  )
...
attendance_per_lecture <- all_data %>%
  group_by(SESSION_NUMBER, YEAR.x) %>%
  summarise(
